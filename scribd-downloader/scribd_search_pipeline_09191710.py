...
def collect_one_result_page(
    ...
    """
    Open one explicit Scribd search result page and collect document links.

    A few short scroll rounds are used only to let all cards on that page render.
    There is no Next-button clicking.
    """
    search_scribd(
        driver,
        keyword,
    ...
    while stable_rounds < settle_rounds:
        ...
        current_count = len(page_documents)

        print(
            f"    page={page_number} "
            f"scan={round_number} "
            f"page_unique={current_count}"
        )
...
def collect_search_results(
    ...
    Stops when:
      ...
      - or two consecutive explicit pages produce no new document IDs.
    """
    documents = {}
