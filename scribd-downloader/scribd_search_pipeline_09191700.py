...
def build_search_url(keyword):
    """
    Build a Scribd search URL with the 1-3 page filter applied directly.
    """
    params = {
        "query": keyword,
        "ct_lang": "0",
...
def search_scribd(driver, keyword):
    """
    Open Scribd's filtered search URL directly.
    """
    search_url = build_search_url(keyword)

    print(f"[SEARCH] {keyword}")
    print("[FILTER] 1-3 pages")
    ...
    print(f"[INFO] Loaded: {driver.current_url}")
...
def find_next_page_control(driver):
    """
    Find a visible/enabled Next-page control.
    """
    selectors = [
    ...
    for by, selector in selectors:
        ...
        for element in elements:
            try:
                ...
                if (
                    (element.get_attribute("aria-disabled") or "").lower()
                    == "true"
                ):
                    continue
...
def current_result_signature(driver):
    items = collect_visible_documents(driver)
...
def click_next_page(driver):
    """
    Click Next and verify that a different result page loaded.
    """
    control = find_next_page_control(driver)

    if control is None:
        return False

    before_url = driver.current_url
    before_signature = current_result_signature(driver)
    ...
    try:
        ...
        def page_changed(d):
            if d.current_url != before_url:
                return True
            return current_result_signature(d) != before_signature
        ...
        try:
            ...
            if (
                ...
                and current_result_signature(driver) == before_signature
            ):
                return False
...
def collect_search_results(
    ...
    """
    Collect across lazy-loaded chunks and Next-page pagination.
    """
    documents = {}
    page_number = 1
    ...
    while len(documents) < max_results:
        ...
        while len(documents) < max_results:
            ...
            print(
                ...
                f"unique={current_count}/{max_results}"
            )
        ...
        print(
            f"    page={page_number} stalled at "
            f"{len(documents)}; checking Next..."
        )

        if not click_next_page(driver):
            print("    no usable Next page; stopping this keyword")
            break

        page_number += 1

        newly_visible = collect_visible_documents(driver)
        for item in newly_visible:
            doc_id = item["id"]
            if doc_id and doc_id not in documents:
                documents[doc_id] = item

        print(
            f"    moved to page={page_number}; "
            f"unique={len(documents)}/{max_results}"
        )
